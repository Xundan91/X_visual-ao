export const GOLD_SKY_GQL = "https://arweave-search.goldsky.com/graphql"

export const AppVersion = process.env.version || "unknown";
export const AOModule = "Do_Uc2Sju_ffp6Ev0AnLVdPtot15rvMjP-a9VVaA5fM"; // aos 2.0.1
export const AOScheduler = "_GQ33BkPtZrqxA84vM8Zk-N2aO0toNNu_C-l-rawrBA";
export const AOAuthority = "fcoN_xJeisVsPXA-trzVAuIiqO3ydLQxM-L4XbrQKzY";

export const APM_ID = "DKF8oXtPvh3q8s0fJFIeHFyHNM6oKrwMCUrPxEMroak";
export const APM_INSTALLER = "https://raw.githubusercontent.com/betteridea-dev/ao-package-manager/refs/heads/main/installer.lua";

export const CommonTags = [
    { name: "App-Name", value: "Visual-AO" },
    { name: "App-Version", value: AppVersion },
];

export const CommonActions = {
    "Info": "Info",
    "Credit-Notice": "Credit-Notice",
    "Debit-Notice": "Debit-Notice",
    "Balance": "Balance",
}
